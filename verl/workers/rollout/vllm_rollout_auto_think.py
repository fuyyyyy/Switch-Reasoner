...
def _normalize_eos_id(eos_token_id) -> int:
...
def _default_toolcall_followup_prompt() -> str:
    return (
        "<|im_start|>user\n"
        "I noticed you tried to call a tool. Please do not call any tools.\n"
        "Now You FIRST think about the reasoning process as an internal monologue and then provide the final answer. Put your concise reasoning inside <thinking>...</thinking>, and put the final answer inside <answer>...</answer>.\n"
        "<|im_end|>\n"
        "<|im_start|>assistant"
    )

def get_last_eos_mask(response_ids: torch.Tensor, eos_id: int, pad_id: int, dtype):
    # response_ids: (B, T)
    B, T = response_ids.shape
    device = response_ids.device

    nonpad = (response_ids != pad_id)
    eos_pos = (response_ids == eos_id) & nonpad

    idx = torch.arange(T, device=device).view(1, T).expand(B, T)
    last_eos_idx = (idx * eos_pos.long()).max(dim=1).values  # 0 if none

    has_eos = eos_pos.any(dim=1)
    last_nonpad_idx = (idx * nonpad.long()).max(dim=1).values

    end_idx = torch.where(has_eos, last_eos_idx, last_nonpad_idx)  # (B,)

    mask = (idx <= end_idx.view(B, 1)) & nonpad
...
class vLLMRollout_auto(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        eos_id = _normalize_eos_id(eos_token_id)
        bs = input_ids.size(0)
        ...
        followup_prompt = getattr(self, "toolcall_followup_prompt", _default_toolcall_followup_prompt())
        followup_prompt_ids = self.tokenizer.encode(followup_prompt, add_special_tokens=False)
        ...
        for i, completion in enumerate(completions):
            ...
            for out in completion.outputs:
                ...
                if will_continue:
                    ...
                    full_prompt_ids.extend(list(followup_prompt_ids))
                    resp_genmask.extend([0] * len(followup_prompt_ids))
                ...
                for gid, comp in zip(gid_list, comps):
                    ...
                    for k in range(L):
                        ...
                        if will_continue:
                            ...
                            traj_prompt_token_ids[j].extend(list(followup_prompt_ids))
                            traj_resp_genmask[j].extend([0] * len(followup_prompt_ids))
        ...
        response_mask = get_last_eos_mask(response_ids, eos_id, self.pad_token_id, attention_mask.dtype)
