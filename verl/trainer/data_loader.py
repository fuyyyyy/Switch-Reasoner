...
def create_dataloader(config: DataConfig, tokenizer: PreTrainedTokenizer, processor: Optional[ProcessorMixin],     filter_log_dir: Optional[str] = None) -> None:
    train_dataset = RLHFDataset(
        ...
        user_direct_prompt=config.user_direct_prompt,
        user_think_prompt=config.user_think_prompt,
        min_pixels=config.min_pixels,
        max_pixels=config.max_pixels,
        filter_overlong_prompts=config.filter_overlong_prompts,
        filter_overlong_prompts_workers=config.filter_overlong_prompts_workers,
        filter_overlong_log_path=os.path.join(filter_log_dir, "train_overlong.jsonl") if filter_log_dir else None,
        mode = config.mode,
    )
    ...
    val_dataset = RLHFDataset(
        ...
        max_pixels=config.max_pixels,
        filter_overlong_prompts=config.filter_overlong_prompts,
        filter_overlong_log_path=os.path.join(filter_log_dir, "val_overlong.jsonl") if filter_log_dir else None,
        mode = config.mode
    )
